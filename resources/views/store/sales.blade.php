    ...
    <div class="scroll-sidebar">
        ...
        <nav class="sidebar-nav">
            ...
            <ul id="sidebarnav">
                ...
                <li class="sidebar-item"><a class="sidebar-link waves-effect waves-dark Active sales-button" role="button" aria-expanded="false" style="color:#febf52; font-weight:500;"><i class="fas fa-piggy-bank" aria-hidden="true" style="color:#febf52;"></i><span class="hide-menu">Sales</span><span><i class="fas fa-caret-down rotate"></i></span></a></li>
            ...
            <div class="col-sm-4">
                <div class="white-box">
                    <div class="row">
                        ...
                        <div class="col-6">
                            <a href="add_sales" class="btn btn-outline-primary mb-2 float-right m-l-20  waves-effect waves-light"><i class="fa fa-plus-circle" aria-hidden="true"></i> Add</a>
                        </div>
                    </div>
                    <div class="table-responsive">
                        <table class="table" id="inventory_table">
                            <thead>
                                <tr>
                                    <th class="border-top-0">Daily Sale</th>
                                    <th class="border-top-0">Date</th>
                                    <th class="border-top-0 text-center" width="20%" colspan="2">Action</th>
                                </tr>
                            </thead>
                            <tbody>
                                @foreach($data['daily_sales'] as $daily_sale)
                                <tr>
                                    <td>
                                        @php($item_tot_price = 0.00)
                                        @foreach($data['sales_by_date'] as $sales_by_date)
                                        @if($sales_by_date->transaction->transaction_date == $daily_sale->transaction_date)
                                        @php($item_tot_price = $item_tot_price + $sales_by_date->total_price)
                                        @endif
                                        @endforeach
                                        {{ number_format($item_tot_price, 2, '.', '') }}
                                    </td>
                                    <td>{{ $daily_sale->transaction_date }}</td>

                                    <td class="text-center">
                                        <button class="btn btn-info text-white show_item_list" name="add_inventory" id="add_inventory" data-id="{{ $daily_sale->id }}" data-label=""><i class="fas fa-eye"></i></button>
                                    </td>
                                    <td class="text-center">
                                        <a href="generate_sales_pdf/{{ $daily_sale->transaction_date }}" target="blank"><button class="btn btn-warning text-white" name="generate_sales_pdf" id="generate_sales_pdf" data-id="" data-label=""><i class="far fa-file-pdf"></i></button></a>
                                    </td>
                                </tr>
                                @endforeach
                            </tbody>
                        </table>
                        {{ $data['daily_sales']->links() }}
                    </div>
                </div>
            </div>
            <div class="col-sm-8">
                <div class="white-box">
                    <div class="row">
                        <div class="col-12">
                            <h3 class="box-title">Product Sold</h3>
                        </div>
                    </div>
...
<script>
    $(document).ready(function() {
        $('.show_item_list').on('click', function() {
            ...
            $.ajax({
                url: "{{ url('store/show_item_list') }}",
                method: 'post',
                data: {
                    transaction_date: transaction_date
                },
                success: function(response) {
                    $('#show_products').html(response.inv_data);
